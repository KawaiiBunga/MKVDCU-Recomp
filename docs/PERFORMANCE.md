...
| CPU | Keep CTR, XER, CR, reserved and non-argument PowerPC registers in C++ locals (`config/mkvsdcu_codegen.toml`). The host compiler can then keep them in registers instead of reloading the context struct. | done, measure | Less CPU per guest instruction on the two saturated guest threads |
